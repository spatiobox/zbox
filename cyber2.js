...
async function main() {
    ...
    let sec3 = [];
    let sec4 = [];
    let positions = Array.from(new Array(50), (x, i) => i / 50).reduce((p, c) => {
        let y = Math.sqrt(r ** 2 - c ** 2);
        let x = c * canvas.height / canvas.width;
        p.push(x, y);
        sec2.unshift(-x, y);
        sec3.push(-x, -y);
        sec4.unshift(x, -y);
    ...
    // });
    ctx.uniform4f(colorUniformLocation, Math.random(), Math.random(), Math.random(), Math.random());
    ctx.drawArrays(ctx.TRIANGLE_FAN, 0, 200);
